...
def test_pending_ttl_and_pop():
    pending_put(111, "нужен бот")
    assert pending_get(111) == "нужен бот"
    PENDING_CLARIFY[222] = ("старое", 0.0)  # протухшая запись (monotonic=0 — давно)
    assert pending_get(222) is None
    assert pending_pop(333) is None
    assert pending_pop(111) == "нужен бот"
    assert pending_get(111) is None
